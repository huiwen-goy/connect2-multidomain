...
- General Relationship Satisfaction: Total score
...
rs$RelSatisf_total <- rowMeans(rs[, c(2:8)], na.rm=TRUE)
rs$RelSatisf_total[is.nan(rs$RelSatisf_total) == TRUE] <- NA
...
Note: All those with unknown Group and all those with >10% missing data have been excluded in these plots.
```{r, echo=FALSE}

# Set order of group factor levels
data$Group <- factor(data$Group, levels = c("NH", "HL", "HA", "NA"))

# Remove those with unknown Group and >10% missing data; leaves 424
data2 <- data[is.na(data$Group) == FALSE & data$Include_over90data == 1, ]
...
**Overall responses across comorbidity items**
```{r, echo=FALSE}
...
**Percentage of 'yes' responses for each group**
```{r, echo=FALSE}
...
**Mean number of each type of comorbidity reported, by hearing group**
...
# Initial suggestion: vision, "physical", arthritis, neuro/cognitive, psychological
#names(data2)[22:52]

# My groups
data2$Count_vision <- rowSums(data2[, c('cataracts', 'eye')] == 1)
data2$Count_cardio <- rowSums(data2[, c('heart', 'vascular', 'highbp', 'stroke')] == 1)
data2$Count_arthritis <- rowSums(data2[, c('rheumatoid', 'arthritis')] == 1)
data2$Count_diabetes <- ifelse(data2[, 'diabetes'] == 1, 1, 0)
data2$Count_mobility <- rowSums(data2[, c('osteoarth', 'back', 'osteoporosis')] == 1)
data2$Count_memory <- rowSums(data2[, c('memory', 'dementia')] == 1)
data2$Count_neuro <- rowSums(data2[, c('parkinson', 'ms', 'epilepsy', 'migraine')] == 1)
data2$Count_psych <- rowSums(data2[, c('anxiety', 'depression')] == 1)
data2$Count_other <- rowSums(data2[, c('allergies', 'asthma', 'lung', 'hypothyroid', 'hyperthyroid', 'cancer', 'kidney', 'urinary', 'ulcers', 'bowel.disorder', 'bowel.incont')] == 1)

# Subsetting by hearing status, again
NH <- subset(data2, Group == "NH")
HL <- subset(data2, Group == "HL")
HA <- subset(data2, Group == "HA")

# Mean number of each type of comorbidity reported, by group
round(data.frame( 
  NH = colMeans(NH[, 337:345], na.rm=TRUE), 
  HL = colMeans(HL[, 337:345], na.rm=TRUE), 
  HA = colMeans(HA[, 337:345], na.rm=TRUE) ), digits=2 )
...
Connect1_Connect2_merged_2020.07.16.csv
```{r, echo=FALSE}

### Merging Connect 1 and Connect 2 data

# Read Connect 2 dataset, with additional variables already built in
#data <- read.csv("Qualtrics_data_2020.07.13.additional.csv", header=TRUE, na.strings=c("", "NA"))
...
#c4 <- c3[, -c(275:301, 331:333, 367, 373, 444)]
#write.csv(c4, "~/Desktop/Connect1_Connect2_merged_2020.07.16.csv")

# Read merged dataset; columns have been cleaned up so there are no longer duplicates.
cdata <- read.csv("Connect1_Connect2_merged_2020.07.16.csv", header=TRUE, na.strings=c("", "NA"))

```
  
# Characteristics of early and late hearing aid adopters
  
Who purchased HA in Connect 1, and who purchased HA by Connect 2?  
```{r, echo=FALSE}

# addmargins(table(cdata$HA.Purchase, cdata$Use_HA, dnn = c('Connect1', 'Connect2'), useNA = 'always'))
```
![](HA_table.jpg)  

The "control" group was refined by selecting only people whose better ear PTA4 was > 25 dB HL, resulting in n=93 remaining, down from n=337 originally.
  
```{r, echo=FALSE}

# Create four groups
cdata$HA_group <- rep(NA, 424)
cdata$HA_group[cdata$HA.Purchase == "Yes" & cdata$Use_HA == 1] <- "Early_adopt"
cdata$HA_group[cdata$HA.Purchase == "No" & cdata$Use_HA == 1] <- "Late_adopt"
cdata$HA_group[cdata$HA.Purchase == "Yes" & is.na(cdata$Use_HA) == TRUE] <- "Drop_out"
cdata$HA_group[cdata$HA.Purchase == "No" & is.na(cdata$Use_HA) == TRUE & cdata$PTA4_better_ear > 25] <- "Control"

table(cdata$HA_group, cdata$HA.Purchase, useNA='always', dnn = c('Connect2', 'Connect1'))
```

```{r, echo=FALSE, message = FALSE, warning = FALSE}

# PTA4 better ear
ggplot(cdata[is.na(cdata$HA_group)==FALSE & cdata$HA_group != "Drop_out", ], aes(x = PTA4_better_ear, fill = HA_group)) + 
  geom_density(alpha=0.25) + 
  xlab('PTA4 better ear (dB HL)') +
  ylab('Density') + 
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 15), text = 
...
# MOS Social Support
ggplot(cdata[is.na(cdata$HA_group)==FALSE & cdata$HA_group != "Drop_out", ], aes(x = SocSuppMOS_overall, fill = HA_group)) + 
  geom_density(alpha=0.25) + 
  xlab('MOS Social Support overall') +
  ylab('Density') + 
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 15), text = 
      element_text(size = 20))
...
#"Early_adopt": Mean 30.23, 10.00 to 48.75
#describe(cdata$PTA4_better_ear[cdata$HA.Purchase == "Yes" 
                          #    & cdata$Use_HA == 1
                          #    & is.na(cdata$HA.Purchase) == FALSE
                          #    & is.na(cdata$Use_HA) == FALSE])
#"Late_adopt": Mean 31.94, 17.50 to 46.25
#describe(cdata$PTA4_better_ear[cdata$HA.Purchase == "No" 
                           #   & cdata$Use_HA == 1
                           #   & is.na(cdata$HA.Purchase) == FALSE
                           #   & is.na(cdata$Use_HA) == FALSE])
#"Drop_out": 41 dB HL (Note LE2000 is 34; typo)
#describe(cdata$PTA4_better_ear[cdata$HA.Purchase == "Yes" 
                            #  & is.na(cdata$Use_HA) == TRUE
                            #  & is.na(cdata$HA.Purchase) == FALSE])
# Controls with PTA4 better ear > 25, n=93, Mean 32.18, 26.25 to 50.00
#describe(cdata$PTA4_better_ear[cdata$HA.Purchase == "No" 
                           #  & is.na(cdata$Use_HA) == TRUE
                           #  & is.na(cdata$HA.Purchase) == FALSE
                           #  & cdata$PTA4_better_ear > 25])

# Social support scores of each group

#"Early_adopt": Mean 4.162, 1.947368 to 5.000000
#describe(cdata$SocSuppMOS_overall[cdata$HA.Purchase == "Yes" 
                           #   & cdata$Use_HA == 1
                           #   & is.na(cdata$HA.Purchase) == FALSE
                           #   & is.na(cdata$Use_HA) == FALSE])
#"Late_adopt": Mean 4.116, 2.421053 to 5.000000
#describe(cdata$SocSuppMOS_overall[cdata$HA.Purchase == "No" 
                            #  & cdata$Use_HA == 1
                            #  & is.na(cdata$HA.Purchase) == FALSE
                            #  & is.na(cdata$Use_HA) == FALSE])
#"Drop_out": 4.789
#describe(cdata$SocSuppMOS_overall[cdata$HA.Purchase == "Yes" 
                           #   & is.na(cdata$Use_HA) == TRUE
                           #   & is.na(cdata$HA.Purchase) == FALSE])
#Controls with PTA4 better ear > 25, n=93, Mean 3.875, 1.000000 to 5.000000
#describe(cdata$SocSuppMOS_overall[cdata$HA.Purchase == "No" 
                          #   & is.na(cdata$Use_HA) == TRUE
                          #   & is.na(cdata$HA.Purchase) == FALSE
                          #   & cdata$PTA4_better_ear > 25])
...
# Is there an effect of Group on Social Support scores?

# Controls 3.87 vs Early Adopters 4.16; p-value = 0.06781
t.test(
  x = cdata$SocSuppMOS_overall[cdata$HA_group == "Control" & is.na(cdata$HA_group) == FALSE], 
  y = cdata$SocSuppMOS_overall[cdata$HA_group == "Early_adopt" & is.na(cdata$HA_group) == FALSE],
  paired = FALSE, alternative = c("two.sided") )
# Controls vs Late Adopters (note only 20)
t.test(
  x = cdata$SocSuppMOS_overall[cdata$HA_group == "Control" & is.na(cdata$HA_group) == FALSE], 
  y = cdata$SocSuppMOS_overall[cdata$HA_group == "Late_adopt" & is.na(cdata$HA_group) == FALSE],
  paired = FALSE, alternative = c("two.sided") )
# Early vs Late Adopters (note only 20)
t.test(
  x = cdata$SocSuppMOS_overall[cdata$HA_group == "Early_adopt" & is.na(cdata$HA_group) == FALSE], 
  y = cdata$SocSuppMOS_overall[cdata$HA_group == "Late_adopt" & is.na(cdata$HA_group) == FALSE],
  paired = FALSE, alternative = c("two.sided") )
...
The same question on loneliness was asked in Connect 1 as in Connect 2: "In the past week, how often did you feel lonely? All of the time (4), Occasionally (3), Some of the time (2), Rarely or never (1)." In Connect 2, the responses were also recoded to a binary variable (lonely or not) according to Mick et al. (2018). All 424 cases in Connect 2 have loneliness data available, while 22 of these people are missing loneliness data from Connect 1.
   
**Loneliness status by group, in Connect 2**  
```{r, echo = FALSE}  
...
**Changes across four categories of "loneliness" from Connect 1 to Connect 2**
  
75.6% of people did not change their responses on "loneliness" from Connect 1 to Connect 2. Another 6.6% of people responded that they were less lonely at Time 2, while 12.7% responded that they were more lonely at Time 2. (5.2% were missing Time 1 data, so we can't tell if they changed.)
```{r, echo=FALSE}  

# Loneliness Q
#sum(is.na(cdata$Q55_Corrected)) #22
#sum(is.na(cdata$CLSA_lonely)) #0
#d <- data.frame(Connect1 = cdata$Q55_Corrected, Connect2 = cdata$CLSA_lonely)
#t <- table(d[, 1], d[, 2], dnn = c("Connect 1", "Connect 2"), useNA = 'always')
#addmargins(t)
#round(addmargins(prop.table(t) * 100), digits = 1)
#sum(diag(t)) / 424 * 100
#sum(c(t[2:4, 1], t[3:4, 2], t[4, 3])) / 424 * 100
#sum(c(t[1, 2:4], t[2, 3:4], t[3, 4])) / 424 * 100

plot(jitter(cdata$Q55_Corrected), jitter(cdata$CLSA_lonely), lab = c(4, 4, 4),
     xlab="Time 1 loneliness", ylab="Time 2 loneliness")
```
  
Limiting the data to people in three HA adoption groups (59 early adopters, 20 late adopters, and 93 controls), there are actually more people who report being more lonely at Time 2 than at Time 1, compared to the other way around.

|Group|Same %|Better %|Worse %|NA %|
|-----|-----|-----|-----|-----|
|Control|69.89|9.67|16.12|4.30|
|Early_adopters|77.96|5.08|11.86|5.08|
|Late_adopters|75.00|0.00|15.00|10.00|

```{r, echo=FALSE}

# Just looking at three groups: Control, Early and Late Adopters
cdata2 <- cdata[is.na(cdata$HA_group)==FALSE & cdata$HA_group != "Drop_out", ]

lonely <- cdata2[, c('ID', 'HA_group', 'Q55_Corrected', 'CLSA_lonely')]
names(lonely)[3:4] <- c('Time1', 'Time2')

table(lonely$Time1, lonely$Time2, lonely$HA_group, dnn = c('Time1', 'Time2'))
#Control: same 65, better 9, worse 15, NA 4
#Early_adopt: same 46, better 3, worse 7, NA 3
#Late_adopt: same 15, better 0, worse 3, NA 2

```
  
# Change in perceived hearing handicap from Time 1 to Time 2
  
Limiting the data to people in three HA adoption groups (59 early adopters, 20 late adopters, and 93 controls), there are no differences between groups on HHIE-S (total) and no differences from Time 1 to Time 2.
```{r, echo=FALSE, message = FALSE, warning = FALSE}

hhie <- cdata2[, c('ID', 'HA_group', 'HHIE_total', 'HHIES_total')]
names(hhie)[3:4] <- c('Time.1', 'Time.2')

# Make a longform dataset with Group and Time as repeated measures
hhie2 <- reshape(hhie, idvar = c('ID', 'HA_group'), varying = c('Time.1', 'Time.2'), direction = "long")
names(hhie2)[4] <- c('HHIES_total')
# Make Time as factor
hhie2$time <- as.factor(hhie2$time)

# Table of means, by group
round(tapply(hhie2$HHIES_total, list(Group = hhie2$HA_group, Time = hhie2$time), mean, na.rm=TRUE), digits=1)
# Table of SDs
#round(tapply(hhie2$HHIES_total, list(Group = hhie2$HA_group, Time = hhie2$time), sd, na.rm=TRUE), digits=1)

# Is there an effect of Group?
library(lme4)

m0 <- lmer(HHIES_total ~ (1|ID), data=hhie2, REML=FALSE)

m1a <- lmer(HHIES_total ~ HA_group + (1|ID), data=hhie2, REML=FALSE)
anova(m0, m1a) # no effect of group

m1b <- lmer(HHIES_total ~ time + (1|ID), data=hhie2, REML=FALSE)
anova(m0, m1b) # no effect of time
