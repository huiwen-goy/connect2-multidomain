...
From this point, those with unknown Group and or >10% missing data are excluded.
```{r, echo=FALSE}
...
**Overall responses across co-morbidity items**
```{r, echo=FALSE}
...
# Counting the number of each type of condition, per person
...
round(data.frame( 
  NH = colSums(NH[, 338:345] == 1, na.rm=TRUE) / nrow(NH) * 100, 
  HL = colSums(HL[, 338:345] == 1, na.rm=TRUE) / nrow(HL) * 100, 
  HA = colSums(HA[, 338:345] == 1, na.rm=TRUE) / nrow(HA) * 100), digits=1)
```
