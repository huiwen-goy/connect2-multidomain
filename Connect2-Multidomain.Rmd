...
# recode "gender" (I know, it's not biological sex) to 0 and 1
data2$Sex <- c(0)
...
c4 <- c3[, -c(275:301, 332:334)]
write.csv(c4, "~/Desktop/Connect1_Connect2_merged_2020.07.29.csv")
```
  
Connect1_Connect2_merged_2020.07.29
```{r, echo=FALSE}

cdata <- read.csv("Connect1_Connect2_merged_2020.07.29.csv", header=TRUE, na.strings=c("", "NA"))
```
  
## Length of time for HA decision
  
Examining just the 60 HA participants, 16 did not have a date of HA purchase in Connect 1 even though they reported using HA in Connect 2; they likely got their HA elsewhere. The length of time between "Most recent hearing eval" and "Date of HA purchase" was calculated. ("Date of first visit" frequently had missing values, while "Date of most recent visit" was frequently after the date of purchase, so these were not good time markers.)
  
```{r, echo=FALSE}

ha <- subset(cdata, Group == "HA")
ha$Time_to_purchase <- as.Date(ha$Date.HA.Purchase, format = "%m %d %Y") - as.Date(ha$Date.Most.Recent.Hearing.Eval, format = "%m %d %Y")
ha$Time_to_purchase[ha$Time_to_purchase < 0] <- NA

plot(x = ha$SocSuppMOS_overall, y = ha$Time_to_purchase, xlim = c(1, 5), ylim = c(0, 200), pch = 16, 
     xlab = "MOS Social Support overall", ylab = "Time to purchase (days)")
...
# Calculate how many got better, got worse, or stayed the same, or were NA
...
The proportions of those who were lonely did not seem to differ by hearing status.
