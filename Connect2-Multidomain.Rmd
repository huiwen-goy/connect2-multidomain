...
4. Created Qualtrics ID "key" for later matching with Connect IDs, with five columns: ResponseId, RecipientLastName, RecipientFirstName, Consent_1_main, Consent_2_clinic; file named "id_qualtrics.csv".
...
6. Match Connect IDs to Qualtrics IDs using first and last names.
...
# Connect names & id
#connect <- read.csv("Name_ID.csv", header=TRUE, na.strings=c(""))
...
# Final result: Of 527 respondents who consented to participate, 453 matched, 6 declined to share info, 2 did not fill out consent option to share info, 66 unmatched to Connect IDs.

```

7. Match audiometric and other clinic info to Qualtrics data using Qualtrics IDs.
```{r, echo=FALSE}

# Qualtrics data
#qdata <- read.csv("Multidomain_2020.07.02_18.16_numeric.csv", header=TRUE, na.strings=c(""))

# ID key
#id <- read.csv("id_qualtrics_connect.csv", header=TRUE)
#id <- id[, -c(2, 3, 5)]

# Clinic data
#cdata <- read.csv("clinic.csv", header=TRUE, na.strings=c(""))
...
Of 527 respondents: 444 (84%) consented to share their clinic data and had clinic data available; 75 (14%) consented to share but their Connect ID did not match a Connect database ID; 6 (1%) did not consent to share, 2 (<1%) left the clinic data consent option blank.

```{r, echo=FALSE}

# Read dataset containing Qualtrics data matched to Connect data for those who consented to sharing clinic data

data <- read.csv("Qualtrics_data_2020.07.06.csv", header=TRUE)

# Count of respondents with matching clinic data
#d2 <- data[, c('Consent_2_clinic', 'ConnectId', 'ID.clinic')] #527
#nrow(d2[d2$Consent_2_clinic == 1 & is.na(d2$ID.clinic) == FALSE,]) #444
#nrow(d2[d2$Consent_2_clinic == 1 & is.na(d2$Consent_2_clinic) == FALSE & #is.na(d2$ID.clinic) == TRUE,]) #75
#nrow(d2[d2$Consent_2_clinic == 2 & is.na(d2$Consent_2_clinic) == FALSE,]) #6
#nrow(d2[is.na(d2$Consent_2_clinic) == TRUE & is.na(d2$ID.clinic) == TRUE,]) #2

# Conclusion: Of 527 respondents, 444 (84%) consented to share their clinic data and had clinic data available; 75 (14%) consented to share but their Connect ID did not match a Connect database ID; 6 (1%) did not consent to share, 2 (<1%) left the clinic data consent option blank.

```

Of those 444 who consented to share their clinic data and had clinic data available, 439 have full audiograms available, and 5 have between 2 and 6 missing cells in their audiograms.
```{r, echo=FALSE}

# Evaluate missing data: Audiograms

# 444 respondents with available clinic data
#d2 <- data[data$Consent_2_clinic == 1 & is.na(data$ID.clinic) == FALSE, ]
# 439 have full audiogram available, 5 have missing cells in audiogram
#table(rowSums(is.na(d2[, 289:300])))

```

Evaluating only the items in the questionnaire that make up the shortest possible path through the survey, i.e., skipping all conditional items that only appear given a 'yes' answer earlier, 93.7% of repondents answered at least 90% of questionnaire items. 
```{r, echo=FALSE}
...
# Cumulative table of available (non-NA) data
...
**List of all new variables added to existing 300**
```{r, echo=FALSE}
str(data[, 301:332])
...
### Number & percentage of participants in each group
Those who are "NA" are not classified because there is no PTA4 information available for them.
```{r, echo=FALSE}
table(data$Group, useNA = "always")
round(table(data$Group, useNA = "always") / 527 * 100, digits=1)

# Why are some participants not classified? Missing audiogram data.
#data2 <- data[is.na(data$Group), 285:332]
...
After cleaning, age is known for 522 respondents and unknown for 5 respondents.
"Age" is added as a new variable.
...
sum(is.na(data$Age))
hist(data$Age, xlim = c(50, 100), breaks = 20, 
  main = "", xlab = "Age at study (years)", col = "grey")

```

### Descriptive plots
 
Note: All those with unknown Group have been excluded for these plots.
 
```{r, echo=FALSE, message = FALSE, warning = FALSE}

library(ggplot2)

# Note: For now, exclude all with unknown Group
data$Group <- factor(data$Group, levels = c("NH", "HL", "HA", "NA"))

# Age
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = Age, fill = Group)) + 
...
# Comorbidities
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = Comorb_count, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = SSQ15i_avg, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = SIM_total, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = HHIES_soc, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = HHIES_emo, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = CSRQ_total, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
# ABC balance scale
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = ABC_average, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
# Life satisfaction
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = SWLS_average, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = WHOQOL_Q1_qol, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = WHOQOL_D1, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = WHOQOL_D2, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = WHOQOL_D3, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = WHOQOL_D4, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
# PHQ4
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = PHQ4_total, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
**Loneliness (re-coded as binary variable)**
```{r, echo=FALSE, message = FALSE, warning = FALSE}

# Loneliness
lonely <- data.frame(Group = data[is.na(data$Group) == FALSE, ]$Group, 
  Lonely = data[is.na(data$Group) == FALSE, ]$Lonely)
lonely$Status <- factor(data[is.na(data$Group) == FALSE, ]$Lonely, 
  levels=c(0, 1), labels=c("Not lonely", "Lonely"))
#table(lonely$Group, lonely$Status)
round(prop.table(table(lonely$Group, lonely$Status), margin=1), digits=3) * 100
```
 
**Social participation (re-coded as binary variable)**
 
Social participation: Including all participants
```{r, echo=FALSE, message = FALSE, warning = FALSE}

# Social Participation
socpart <- data.frame(Group = data[is.na(data$Group) == FALSE, ]$Group, 
  Participation = data[is.na(data$Group) == FALSE, ]$Participation)
socpart$Status <- factor(data[is.na(data$Group) == FALSE, ]$Participation, 
...
# Social Participation
socpart2 <- data[is.na(data$Group) == FALSE & data$Demo_3_employ == 2, c('Group', 'Participation')]
...
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = SocSuppMOS_overall, fill = Group)) + 
  geom_density(alpha=0.25) + 
...
# PTA4 better ear
ggplot(data[is.na(data$Group) == FALSE, ], aes(x = PTA4_better_ear, fill = Group)) + 
  geom_density(alpha=0.25) + 
